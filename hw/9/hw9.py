...
def catenateLoop(str_list):
    ...
    for string in str_list:
        catenatedString = catenatedString + string
...
def wordScoreLoop(S, scorelist):
    ...
    score = 0
    for letter in S:
        score = score + letterScore(letter, scorelist)
    return score
...
def wordsWithScoreLoop(dct, scores):
    ...
    output = []
    for word in dct:
        output = output + [[word, wordScore(word, scores)]]
    return output
